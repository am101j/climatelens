...
const Report = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE}/report/preview`, {
        method: "POST",
      ...
      const data: ClimatePreview = await response.json();
      setPreview(data);
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/report/download?address=${encodeURIComponent(preview.address)}`
      );
      ...
      a.href = url;
      a.download = `${preview.address}_ClimateReport.pdf`;
